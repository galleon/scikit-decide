...
class MultiAgentMaze(D):
    ...
    def _generate_agents(self):
        ...
        left_agents_1_starts = [tuple(map(op.add, divmod(i, quadrant_ydim), (1, 1))) for i in left_agents_1]
        left_agents_2 = rd.sample(quadrant, self._nb_agents // 4)
        left_agents_2_starts = [tuple(map(op.add, divmod(i, quadrant_ydim), (1, quadrant_ydim + 4))) for i in left_agents_2]
        left_agents_starts = left_agents_1_starts + left_agents_2_starts
        right_agents_1 = rd.sample(quadrant, self._nb_agents // 4)
        right_agents_1_starts = [tuple(map(op.add, divmod(i, quadrant_ydim), (quadrant_xdim + 2, 1))) for i in right_agents_1]
        right_agents_2 = rd.sample(quadrant, self._nb_agents // 4)
        right_agents_2_starts = [tuple(map(op.add, divmod(i, quadrant_ydim), (quadrant_xdim + 2, quadrant_ydim + 4))) for i in right_agents_2]
        ...
        self._agents_starts = {'Agent #{}'.format(i): AgentState(x=p[0], y=p[1]) for i, p in enumerate(self._agents_starts)}
        self._agents_goals = left_agents_goals + right_agents_goals
        self._agents_goals = {'Agent #{}'.format(i): AgentState(x=p[0], y=p[1]) for i, p in enumerate(self._agents_goals)}
        ...
        if memory[agent] == self._agents_goals[agent]:
            ...
            for other_agent, other_agent_action in other_agents_actions.items():
                if other_agent_action == AgentAction.left:
                    ...
                    occupied_next_cells.add((memory[other_agent].x, memory[other_agent].y - 1))
            ...
            if memory[agent].y < self._num_rows - 1 and \
                self._maze[memory[agent].y + 1][memory[agent].x] == 1 and \
                (memory[agent].x, memory[agent].y + 1) not in occupied_next_cells:
                applicable_actions.append(AgentAction.down)
            if memory[agent].x > 0 and \
                self._maze[memory[agent].y][memory[agent].x - 1] == 1 and \
                (memory[agent].x - 1, memory[agent].y) not in occupied_next_cells:
                applicable_actions.append(AgentAction.left)
            if memory[agent].x < self._num_cols - 1 and \
                self._maze[memory[agent].y][memory[agent].x + 1] == 1 and \
                (memory[agent].x + 1, memory[agent].y) not in occupied_next_cells:
...
class D(Domain, SingleAgent, Sequential, UncertainTransitions, Actions, Initializable,
        Markovian, PositiveCosts, Goals, FullyObservable):
    ...
    def _get_next_state_distribution(self, memory: D.T_memory[D.T_state],
        ...
        if action == AgentAction.left:
            if memory.x > 0 and self._maze[memory.y][memory.x - 1] == 1:
                ...
                if memory.y < self._num_rows - 1 and self._maze[memory.y + 1][memory.x - 1] == 1:
                    next_state_2 = AgentState(x=memory.x - 1, y=memory.y + 1)
        ...
        if action == AgentAction.up:
            if memory.y > 0 and self._maze[memory.y - 1][memory.x] == 1:
                ...
                if memory.x > 0 and self._maze[memory.y - 1][memory.x - 1] == 1:
                    next_state_2 = AgentState(x=memory.x - 1, y=memory.y - 1)
                if memory.x < self._num_cols - 1 and self._maze[memory.y - 1][memory.x + 1] == 1:
                    next_state_2 = AgentState(x=memory.x + 1, y=memory.y - 1)
        if action == AgentAction.down:
            if memory.y < self._num_rows - 1 and self._maze[memory.y + 1][memory.x] == 1:
                ...
                if memory.x < self._num_cols - 1 and self._maze[memory.y + 1][memory.x + 1] == 1:
                    next_state_2 = AgentState(x=memory.x + 1, y=memory.y + 1)
        return DiscreteDistribution([(next_state_1, 0.8), (next_state_2, 0.1), (next_state_3, 0.1)])
